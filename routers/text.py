...
# Strong references to in-flight Lane 1 + Lane 2 background tasks. The event
# loop only holds weak references to tasks created via asyncio.create_task,
# so a task can be garbage-collected mid-execution if no other reference
# exists. Holding the task here (with discard-on-done) keeps it alive until
...
async def clean_text(body: TextCleanRequest, request: Request):
    ...
    # Async Fork — Lane 1 (publishing) and Lane 2 (intelligence) run as
    # fire-and-forget background work. The HTTP response returns as soon as
    # cleaned_text is available; Lane 1 + Lane 2 land in Postgres / Kinesis /
    # EventBridge on the event loop without blocking the response.
    #
    # Why: Lane 2 (LLM intelligence extraction) takes 100-160s in production.
    # Awaiting it on the response path caused Railway's edge proxy (~300s
    # hard cap) to drop the client TCP under sustained load — server-side
    # writes succeeded but the client saw RemoteProtocolError, producing
    # split-brain state (see tests/integration/test_text_clean_response_decoupling.py
    # for the contract this preserves). The /upload/complete endpoint uses
    # the same asyncio.create_task pattern (routers/upload.py:331).
    ...
    async def _lane1_publish() -> Optional[dict]:
        ...
        try:
            ...
            logger.error(
                f"Lane 1 (publishing) error: interaction_id={interaction_id_str}, error={e}"
            )
            raise
    ...
    async def _lane2_intelligence() -> Optional[object]:
        ...
        try:
            ...
            logger.error(
                f"Lane 2 (intelligence) error: interaction_id={interaction_id_str}, error={e}"
            )
            raise

    async def _run_background_lanes() -> None:
        """Run Lane 1 + Lane 2 concurrently with error isolation. Logs only."""
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                logger.error(
                    f"{lane_name} failed (non-critical): "
                    f"interaction_id={interaction_id_str}, "
                    f"error={type(result).__name__}: {str(result)}",
                    exc_info=result,
                )
            elif i == 0 and isinstance(result, dict):
                logger.info(
                    f"Envelope published: interaction_id={interaction_id_str}, "
                ...
                logger.info(f"{lane_name} completed: interaction_id={interaction_id_str}")
        logger.info(
