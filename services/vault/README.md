...
8. **eq-frontend comments-generator multiSchema fix** (BLOCKING Phase 2a deploy — next session's first task). `@onozaty/prisma-db-comments-generator@1.5.0` emits unqualified `COMMENT ON COLUMN user_credentials...` instead of `vault.user_credentials` for `@@schema("vault")` models. Vercel preview build on eq-frontend#418 fails at `prisma migrate deploy`. Resolution path (next session's `/investigate` scope): (a) upgrade generator to 1.7.0 and retest; (b) configure generator to skip `vault.*` models if a flag exists; (c) patch the generator's DMMF integration to populate `model.schema` correctly for multiSchema; (d) switch to a different generator that handles multiSchema; (e) disable the generator and check in static comment SQL. Track resolution under Linear EQ-11 (eq-frontend schema drift / multiSchema brittleness) or as a new linked issue.
