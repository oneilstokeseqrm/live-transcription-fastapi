...
8. **eq-frontend comments-generator multiSchema fix** ✅ **RESOLVED 2026-05-24 via Path (b) — ignorePattern config.** Root cause turned out to be deeper than initially hypothesized: Prisma 5.22's CLI emits DMMF JSON to generators WITHOUT a `schema` field on Model objects, even for `@@schema()`-annotated multiSchema models (verified empirically via probe generator — full 2.3M-line DMMF contained zero `"vault"` references outside the datasources block). So `model.schema ?? void 0` always reads undefined → unqualified SQL → fails for vault tables (works for public tables via `search_path`). The generator's own code IS multiSchema-aware; the gap is upstream in Prisma 5.22's DMMF design. Fix shipped as eq-frontend PR #418 commit 7905222 (squash-merge): added `ignorePattern = "^(user_credentials|credential_access_log)$"` to the `generator dbComments` block in schema.prisma. Vault tables skip COMMENT ON SQL generation entirely; their rich docstrings live in schema.prisma + this README. The deeper-root-cause fix (upgrade Prisma to 7.x, which DOES emit `model.schema`) is tracked under Linear EQ-11 — out of scope for unblocking Phase 2a. New tasks/lessons.md entries codify the DMMF gap + the related P3009 leftover-failed-migration pattern.
...
| 2026-05-23 (Phase 2b Codex R7) | peteroneil + Claude | **GATE PASSED** — 0 P1 findings. Folded R7 P2: `_get_kms_client()` now runs INSIDE the try block in both `encrypt_credential` and `decrypt_credential`, so boto session-construction failures (e.g., `ProfileNotFound` when `AWS_PROFILE` is set wrong) also surface as structured `VaultError` instead of leaking raw exceptions. 71 tests pass. **Phase 2b shipped** — Codex pre-merge gate clean after 7 rounds. |
| 2026-05-24 (Phase 2b MERGED + DEPLOYED) | peteroneil + Claude | PR #24 squash-merged to main as commit `7f98920`. Railway deployment `2ce20b0e-9de3-42a5-ac66-af4cbef982d6` SUCCESS. `/health` 200 verified. Vault module loaded as inert code (nothing imports it yet so no production code path runs). Companion eq-frontend PR #418 (Phase 2a Prisma migration) BLOCKED on `@onozaty/prisma-db-comments-generator@1.5.0` multiSchema bug — Vercel preview fails because the auto-generated `update_comments` migration emits unqualified `user_credentials` instead of `vault.user_credentials`. Resolution path documented in Phase 2.1 hardening item #14. KMS smoke test deferred until eq-frontend deploys (needs the real tables). |
| 2026-05-24 (eq-frontend#418 UNBLOCKED + MERGED + KMS SMOKE TEST PASSED) | peteroneil + Claude | Root-caused the comments-generator bug via empirical probe-generator dump of Prisma 5.22 DMMF (showed `model.schema` is omitted entirely — see Phase 2.1 hardening item #14 above for full forensics). Applied minimum-blast-radius fix: `ignorePattern` in schema.prisma generator block. Pushed fix as eq-frontend `42e7d48`; resolved leftover failed-migration record in production Neon's `_prisma_migrations` via `UPDATE ... SET rolled_back_at = NOW()` on row id `944fe722-58a4-4d07-99d6-1c321bee058f`; pushed empty retrigger commit `8a41dc7`; Vercel preview SUCCESS; admin-override squash-merged as `7905222` on eq-frontend main. Vercel production deploy READY (deployment `2he8eDSfSLdapZ1eRXa6mSpjJkdq`). Production Neon migration timeline clean: 2 new successful `update_comments` migrations applied (`20260524101648` from preview, `20260524103705` from production deploy); old failed `20260523235715_update_comments` correctly marked `rolled_back_at`. Vault schema + 3 tables verified live in production with correct column counts + FKs + UNIQUEs + indexes. KMS smoke test PASSED end-to-end via local `boto3` with `EQ_VAULT_*` credentials fetched from Railway env vars: positive (4-field EncryptionContext → GenerateDataKey success, 32-byte plaintext + 184-byte CiphertextBlob), negative (3-field, missing credential_id → AccessDeniedException — LOCKED-40 enforced by IAM policy), identity check (ARN matches `arn:aws:iam::211125681610:user/eq-vault-service`). Phase 2 foundation fully verified end-to-end. 4 new lessons codified in tasks/lessons.md. Pre-existing live-db CI check still failing (DIRECT_DATABASE_URL gap — separate scope under Linear EQ-11 family). |
