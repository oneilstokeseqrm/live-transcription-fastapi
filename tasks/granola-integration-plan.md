...
16. **`_persist_intelligence` re-ingest non-idempotency (found 2026-05-26 during the contact-resolution E2E):** `interaction_summary_entries` has a UNIQUE on `(tenant_id, interaction_id, persona_id, level)`, but `IntelligenceService._persist_intelligence` (`services/intelligence_service.py` ~`:102`/`:411`) INSERTs the summary entries WITHOUT `ON CONFLICT`. Re-ingesting an ALREADY-summarized interaction throws `UniqueViolationError`; because `_persist_intelligence` re-raises (unlike `_persist_contact_links`, which swallows), `process_transcript`'s `if contact_ids:` → `_persist_contact_links` (`:120`) is NEVER reached → no `raw_interactions` / `interaction_summaries` / `interaction_contact_links` written. Shared Lane 2 (NOT touched by the Granola contact-resolution PR #36); affects ALL ingestion sources. NORMAL fresh Scenario A + Scenario-C-after-approval are UNAFFECTED (the interaction was never summarized before). Surfaced when re-testing the contact fix on the already-ingested test meeting (`bca60296`) — required a clean-reset (delete the prior summary_entries + insights) to verify. Fix options: (a) make the summary-entry write idempotent (`ON CONFLICT DO UPDATE/NOTHING`), or (b) decouple `_persist_contact_links` so a summary-entry duplicate doesn't block the FK chain. Estimated 0.5 day. **Touches shared Lane 2 → careful review; must NOT break /text/clean, /upload, /batch.** Ticketed 2026-05-26.
17. **`/map` returns 500 (not 503) on the missing-`raw_interactions` ValueError (pre-existing; noted 2026-05-26):** `materialization.py:66-83` comment claims `/map` returns 503 (client retries) when `CHECK_RAW_INTERACTION_EXISTS_SQL` raises `ValueError`, but `routers/queue_actions.py` `/map` has no `ValueError`→503 translation, so it surfaces as a raw HTTP 500. The DBOS `/approve` path DOES retry (it's a `@DBOS.step`). Pre-existing, low-impact (the comment is inaccurate; `/approve` is the resilient path). Fix: add a `ValueError`→503 handler on `/map` OR correct the comment. Estimated 0.25 day. Ticketed 2026-05-26.
18. **Granola Lane 2 fire-and-forget residual (acknowledged 2026-05-26, contact-resolution PR #36):** Lane 2 (`text_clean_service.process` → `IntelligenceService`) is fire-and-forget and `_persist_contact_links` swallows exceptions, so if Lane 2 crashes/fails after Scenario A is marked `success`, the `raw_interactions`/`interaction_contact_links` write can be lost while the run shows `success`. The contact row (synchronous, in the adapter) + the published envelope (sync Lane 1 → downstream Neo4j) ARE durable; only the Postgres link table is best-effort. SAME fragility the transcript path already has (not Granola-specific); far better than the pre-fix state (which failed always). Hardening idea: make the FK-chain write durable (outbox / retry) — bigger shared-Lane-2 change. Related to #16. Estimated TBD. Ticketed 2026-05-26.
19. **Synchronous boto3/KMS calls block the event loop (found 2026-06-05 during the B3-kickoff `/health` 502 investigation):** three hot-path AWS SDK calls run SYNCHRONOUSLY inside `async def` functions with NO `asyncio.to_thread` offload — `services/aws_event_publisher.py` `_publish_to_eventbridge` `put_events` (`:240`) + `_publish_to_kinesis` `put_record` (`:168`, also `publish_batch_completed_event` `put_events:478`), and `services/vault/encryption.py` `decrypt_credential` → `client.decrypt` (`:228`, called at `services/vault/user_credentials.py:510`). While any of these runs on an event loop, that loop is frozen for the full AWS round-trip. **The reference fix already exists in-repo:** `services/account_provisioning/eventbridge_emit.py:245,519` wraps `put_events` in `await asyncio.to_thread(client.put_events, ...)` (Codex 2026-05-16 R6) — these three files were just missed. **Impact:** the `/text/clean` FastAPI handler runs `text_clean_service.process` → `await publisher.publish_envelope` ON THE UVICORN HTTP LOOP, so each `/text/clean` (and the cold-start warm-up) briefly freezes `/health` → the intermittent `502 "Application failed to respond"` observed 2026-06-05 (benign cold-start transient; service was 490/490 clean once warm). **NOT a B3 blocker:** Granola ingestion (incl. B3's background import) runs on DBOS's `BackgroundEventLoop` (separate thread; `dbos/_event_loop.py`), so a multi-hundred-note import blocks the DBOS loop, not `/health` (the GIL is released during boto3 socket I/O). This is hygiene + removes the `/text/clean` web-loop stall. **Fix:** wrap the three sync calls in `asyncio.to_thread` mirroring `eventbridge_emit.py`; add AsyncMock unit tests asserting the offload; run `verify_consumer_contracts.py`. ~10-20 lines. Estimated 0.5 day. **Ticketed by founder decision 2026-06-05 (ship B3 first; this is a fast-follow). Evidence: `~/.gstack` investigate learning `railway-502-warmup-not-ingestion-loop`.**
20. **Per-loop asyncpg pool ownership (Linear EQ-109; found 2026-06-06 in the B3 Codex consult):** `services/asyncpg_pool.py` is a process-global singleton acquired from BOTH uvicorn's HTTP loop AND DBOS's `BackgroundEventLoop` (separate thread). asyncpg pools are loop-bound; cross-loop sharing is a footgun. Pre-existing (Phase 2e); works in prod today (the 5-min poll uses it every cycle). B3 mirrors the proven poll pattern (no new failure mode) + bumps `_DEFAULT_MAX_SIZE` 10→20. Fix: per-loop pool cache (keyed on the running loop) or a dedicated DBOS-loop pool, with re-derived sizing. **Founder decision 2026-06-06: ticket + proceed with B3.** Est. 0.5-1 day.
21. **B3 split-out fast-follows (deferred from EQ-92 per the 2026-06-06 Codex consult):** (a) **newly-added-folder backfill on active-row reconfigure** (plan B3 Step 5b / C17) — bolting `folders_override` + per-folder `created_after` onto `run_one_cycle` worsens the shared-watermark problem; ship B3 for new/reconnect history import + forward anchor only, defer scoped new-folder backfill (true per-folder watermarks are the real fix). (b) **exact re-import progress** — B3's DERIVED progress (COUNT of `external_integration_runs` since `started_at`) is exact for a fresh first import but undercounts on re-runs (created_at is preserved on UPSERT; already-`success` notes short-circuit without re-touching the row). A `granola_import_run_items(import_run_id, external_id, status)` table (idempotent UPSERT) would make re-runs exact. Est. 1 day combined. Ticketed 2026-06-06.
